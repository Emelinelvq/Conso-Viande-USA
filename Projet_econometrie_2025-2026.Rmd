...
```{r, include=TRUE}
# Transformation logarithmique pour obtenir des élasticités
cv_usa <- cv_usa %>%
  mutate(
    ln_CV   = log(CV),
    ln_PIB  = log(PIB),
...
```{r, include=TRUE}
cor(cv_usa[,c("ln_CV","ln_PIB","ln_POP","ln_CPI","ln_BEEF")]) # regarde la multicolinéarité 
cv_usa$ln_CV_pc  <- log(cv_usa$CV / cv_usa$Population)
cv_usa$ln_PIB_pc <- log(cv_usa$PIB / cv_usa$Population)

model_pc <- lm(ln_CV_pc ~ ln_PIB_pc + ln_BEEF, data=cv_usa)
summary(model_pc)
...
cv_usa$ln_PIB_pc <- log(cv_usa$PIB_pc) # on passe en log 
cv_usa$ln_BEEF_real <- log(cv_usa$Beef_real)
# construction du modèle 
model_pc <- lm(ln_CV ~ ln_PIB_pc + ln_BEEF_real, data=cv_usa)
summary(model_pc)
vif(model_pc)
```
...
cv_usa$PIB_pc <- cv_usa$PIB / cv_usa$Population
cv_usa$Beef_real <- cv_usa$Beef_price / cv_usa$CPI
cv_usa$ln_CV_lag <- dplyr::lag(cv_usa$ln_CV, 1)
cv_usa$ln_CV   <- log(cv_usa$CV)
cv_usa$ln_PIB_pc <- log(cv_usa$PIB_pc)
cv_usa$ln_BEEF_real <- log(cv_usa$Beef_real)
model_dyn <- lm(ln_CV ~ ln_CV_lag + ln_PIB_pc + ln_BEEF_real, data=cv_usa)
summary(model_dyn)
dwtest(model_dyn)
...
Le test de Breusch-Pagan indique une hétéroscédasticité, corrigée avec des erreurs standards robustes (type HC1) pour obtenir des tests de significativité fiables.
```{r}
bptest(model_dyn)

# Correction 
coeftest(model_dyn, vcov = vcovHC(model_dyn, type = "HC1"))
```
