...
Nous avons décidé de passer en lo-log afin d'obtenir des coefficients représentant les élasticités. Par ailleurs, nous avons transformé les prix nominaux en prix réels afin de prendre en compte l'impact de l'inflation, ce qui nous semble indispensable dans le cadre d'une analyse de long-terme. 
...
plot(data_reel$Year, data_reel$CV, type="l", main="Log consommation de viande")
```

On observe une baisse de la consommation de viande entre 2008 et 2015, puis une augmentation assez régulière jusqu'en 2024. Nous allons essayer d'expliquer cette rupture de 2015 avec notre modèle. 
...
# Création et résumé de 3 modèles -> pas besoin de mettre les 3 dans le rapport ça va être trop long et inutile je pense 
...
#3 - modèle log-log avec prix réels

modele_reel <- lm(CV ~ PIB + Beef_price, data = data_reel) # modèle régression log-log avec les prix réels

summary(modele_reel)
CV_pred_reel <- predict(modele_reel, newx = X_reel)
r2 <- 1 - sum((CV_reel - CV_pred_reel)^2) / sum((CV_reel - mean(CV_reel))^2)
r2 # 0.4553 et ajusté : 0.3948
```
...
On obtient une valeur de DW bien inférieure à 2 et une p-value inférieure à 0.05, il y a donc une forte autocorrélation. Nous essayons de la corriger grâce à une correction de NeweyWest.
...
modele_reel_dummy_inter <- lm(CV ~ PIB*post2011 + Beef_price*post2011, 
                            data = data_reel)
summary(modele_reel_dummy_inter) # je sais pas trop quoi en penser c'est bizarre comme modèle - pas significatif -> plutot prendre l'autre 

# Si PIB augemente prix de la viande et plus d'influence du PIB que du prix de la viande 
```

## Ajout données prix soja et poulet
Nous avons décidé de rajouter comme variable le prix du soja (global price of soybean) et du poulet car nos résultats précédents ne nous satisfaisaient pas. Les données proviennent de la Federal Reserve Bank of St. Louis. De même, on passe les prix en réel
...
plot(cusum) # On observe une forte rupture de tendance 
# On fait donc un test de Chow pour 2011, date de la rupture
sctest(CV_reel_sojchick ~ X_reel_sojchick, type="Chow", point = 7) # p-value = 8.44e-06, il y a donc bien une rupture temporelle (mais aussi en 2006 et 2005, genre ttes les dates avant 2007 donc à voir quoi en dire et surtout à voir le Cusumsquare)

```

La p-value est inférieure à 0,05, on observe donc bien une rupture temporelle
...
La rupture temporelle a lieu en 2011. On ajoute donc une dummy pour corriger le modèle.
...
L'ajout de Dummy permet d'augmenter la significativité du modèle 
...
On obtient une valeur de DW inférieure à 2 et une p-value inférieure à 0.05, il y a donc une forte autocorrélation. Nous essayons de la corriger grâce à une correction de NeweyWest.
...
```{r}
# Modèle avec données réelles - sans soja 
modele_reel_chick <- lm(CV ~ PIB + Beef_price + prix_poulet, data = data_reel_sojchick) # modèle régression linéaire avec les données en réel

summary(modele_reel_chick)
CV_pred_reel_sojchick <- predict(modele_reel_sojchick, newx = X_reel_sojchick)
# r2
summary(modele_reel_chick)$r.squared       # R² classique
# 0.9080437
summary(modele_reel_chick)$adj.r.squared   # R² ajusté (corrige le nombre de variables)
# 0.8918161
```
